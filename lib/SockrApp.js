...
class SockrApp extends SockrHooks {
  ...
  get connections() {
    let count = 0
    try {
      this.#server.clients.forEach(() => count++ );
    } catch {}
    return count
  }
  ...
  redirect(pattern, callback) {
    if (typeof callback !== "function") throw new TypeError("The callback must be a function")
    let re = pattern instanceof RegExp ? pattern : new RegExp(pattern)
    this.#redirects.push({ re, callback })
  }
  ...
  async #onConnect(websocket, request) {
    ...
    try {
      // check for special redirect/routing 
      for(let r of this.#redirects) {
        if (r.re.test(request.url)) {
          r.callback(websocket, request)
          break
        }
      }
  ...
  async dispatch(context) {
    ...
    if (!context.error) {
      try {
        ...
        // check if undefined as we want 0's, false, empty strings, and empty arrays to be sent
